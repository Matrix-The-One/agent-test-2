...
## 技术选型约定
- 实现新功能前，先确认当前主流实现方案，以及是否已有成熟、维护活跃、与现有技术栈兼容的库可复用。
- 优先选择社区采用广、文档完善、维护状态健康、能减少自研复杂度的方案。
- 不因“流行”而盲目引入新依赖；若现有依赖、平台原生能力或项目内已有实现足够，应优先复用。
- 引入新库时，应简要说明选型理由、替代方案及潜在影响，包括体积、维护成本、兼容性与安全性。
- 涉及新增依赖、核心基础设施替换或高耦合技术选型时，先给出简要方案比较，再实施改动。
